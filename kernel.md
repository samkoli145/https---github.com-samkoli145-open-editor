...
- ⚠️ فجوات أمنية مسجّلة (تتبع في `PLAN.md` §5 ن→ف): TOCTOU بين فحص المسار والتنفيذ · لا عزل نظامي (bubblewrap/Landlock) · توقيع ELF سطحي · REST بلا مصادقة على `0.0.0.0` · `syncToDisk` دمية · قراءة 64B vs 512B · indexer لا يتتبّع هدف symlink · لا رفض setuid في التنفيذ · checksum FNV-1a غير تشفيري.
