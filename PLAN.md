...
|---|---|---|---|
| 1 | **`bootloader` غير معرَّف** في مسارات snowball (نُقل حرفياً من النسخة التجريبية التي كان نطاقها يحوي `bootloader`) — أي استدعاء لـ`/api/snowball/*` يرمي `ReferenceError: bootloader is not defined` | `server.ts:699,707,720` | تحويل المرجع إلى `runtime?.subsystems?.snowball` (مثل بقية المسارات). لا تكشفها الترجمة لأن `server.ts` خارج `include` في tsconfig — نعتمد على الفحص التشغيلي |
